...
function mostrarHoraConvertida() {
    ...
    console.log(hora);
    document.getElementById('tituloResultado').innerHTML = `Titulo de la reunion: ${titulo}`;
    document.getElementById('descripcionResultado').innerHTML = descripcion;
    document.getElementById('linkResultado').innerHTML = link;
    document.getElementById('horaResultado').innerHTML = `La hora en ${paisDestino} es: ${horaEnDestino}`;
...
function convertirHora(hora, paisOrigen, paisDestino) {
    ...
    const horaDestino = fechaOrigen.clone().tz(obtenerZonaHoraria(paisDestino)).format('hh:mm A');
